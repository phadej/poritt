...
batchFile fn = execStateT $ do
  ...
  where
    ...
    lintE pass metas e et = do
        ...
        et' <- either printError return $ evalExceptState (lintElim (emptyLintCtx env.opts.levelCheck names) { metas = metas } e) initialRigidGen
    ...
    lintT pass metas t et = do
        ...
        case evalExceptState (lintTerm (emptyLintCtx env.opts.levelCheck names) { metas = metas } t (coeNoMetasVTerm et)) initialRigidGen of
            Right _  -> pure ()
            Left msg -> printError $ ppVCat
                [ pass <+> "lint failed"
                , msg
    ...
    pipelineElim e = do
        ...
        (metas, (e0, et')) <-
            ...
            then either printError return $
                evalElabM (elabElim (emptyElabCtx opts.levelCheck names) w)
            else either printError return $ do
                res <- evalCheckM (checkElim (emptyCheckCtx opts.levelCheck names) w)
    ...
    pipelineTerm t et = do
        ...
        (metas, t0) <-
            ...
            then either printError return $ do
                evalElabM (elabTerm (emptyElabCtx opts.levelCheck names) w (coeNoMetasVTerm et))
            else either printError return $ do
                res <- evalCheckM (checkTerm (emptyCheckCtx opts.levelCheck names) w (coeNoMetasVTerm et))
                return (emptyMetaMap, res)
    ...
    pipelineEnd t0 et metas = do
        ...
        t1 <- maybe (printError $ "zonk failed") return $ zonkTerm FullZonk (emptyZonkEnv metas) t0
    ...
    stmt (FailStmt e) = do
        ...
        -- elaborate, i.e. type-check
        case evalExceptState (checkElim (emptyCheckCtx opts.levelCheck names) w) initialRigidGen of
    ...
    stmt (OptionsStmt args) = do
        ...
        case parseOptsEndo args of
            ...
            Right endo -> do
                env <- get
                -- GHC record update is resolve in renamer.
                -- https://gitlab.haskell.org/ghc/ghc/-/merge_requests/8686
                put $ (env :: Environment) { opts = endo env.opts }
