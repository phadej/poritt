...
optsP = many $ asum
    ...
    -- level check
    , O.flag' (set #levelCheck True)  $ O.long "stage-check"    <> O.help "Check staging levels (default)"
    , O.flag' (set #levelCheck False) $ O.long "no-stage-check" <> O.help "Don't check staging levels"
