...
sinkLintCtx x' t' (LintCtx vs ts ts' rs ss cs xs xs' ns ms s pp opts) = LintCtx (mapSink vs) (mapSink ts) (mapSink ts' :> sink t') (rigidMapSink (mapSink rs)) ss cs xs (xs' :> x') ns ms (SS s) pp opts

emptyLintCtx :: Bool -> NameScope -> LintCtx pass EmptyCtx EmptyCtx
emptyLintCtx opts ns = LintCtx EmptyEnv EmptyEnv EmptyEnv emptyRigidMap EmptyEnv stage0 EmptyEnv EmptyEnv ns emptyMetaMap SZ [] opts
...
define (LintCtx vs ts ts' rs ss cs xs xs' ns ms s pp opts) x v t = LintCtx (vs :> v) (ts :> t) ts' rs (ss :> cs) cs (xs :> x) xs' ns ms s pp opts

weakenLintCtx :: Wk ctx ctx' -> LintCtx pass ctx' ctx'' -> LintCtx pass ctx ctx''
weakenLintCtx w (LintCtx vs ts ts' rs ss cs xs xs' ns ms s pp opts) = LintCtx (weakenEnv w vs) (weakenEnv w ts) ts' rs (weakenEnv w ss) cs (weakenEnv w xs) xs' ns ms s pp opts
...
lintElim' ctx (Met m xs) = case lookupMetaMap m ctx.metas of
    ...
    Just e  -> lintQruning ctx (sinkSize ctx.size (metaEntryType e)) xs
...
lintQruning :: forall ctx ctx' pass. LintCtx pass ctx ctx'
           -> VTerm pass ctx'
           -> Qruning ctx
           -> LintM (VTerm pass ctx')
lintQruning env ty0 (Qruning wk _TODO) =
    go (weakenEnv wk (tabulateEnv (sizeEnv env.names) id)) ty0
  where
    go :: Env ctx'' (Idx ctx) -> VTerm pass ctx' -> LintM (VTerm pass ctx')
    go EmptyEnv ty = pure ty
