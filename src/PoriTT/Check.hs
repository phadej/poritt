...
--
emptyCheckCtx :: Bool -> NameScope -> CheckCtx EmptyCtx EmptyCtx
emptyCheckCtx opts ns = CheckCtx
    { names  = EmptyEnv
    , names' = EmptyEnv
    , nscope = ns
    , values = EmptyEnv
...
bind (CheckCtx xs xs' ns v ts ts' rs ss cs s wk l pp opts) x x' a = CheckCtx
    { names   = xs :> x
    , names'  = xs' :> x'
    , nscope  = ns
    , values  = mapSink v :> t
...
define
    ...
    -> VTerm HasMetas ctx'   -- ^ type
    -> CheckCtx (S ctx) ctx'
define (CheckCtx xs xs' ns v ts ts' rs ss cs s wk l pp opts) x t a = CheckCtx
    { names   = xs :> x
    , names'  = xs'
    , nscope  = ns
    , values  = v :> t
