...
invert env (VApp sp Ecit t) = do
    ...
    t' <- forceM env.size t
    case match 0 t' of
        Just (m, l) -> case lookupLvlMap l pren of
            -- TODO: i'm not sure if we want lvlZ or lvlTop?
            Nothing -> return $ Invert (SS s') (names :> lookupLvl env l) (insertLvlMap l (Lvl' m (lvlZ s')) (mapSink pren))
            Just _  -> throwError $ "non-linear spine" <+> prettyVTermCtx env t'

        _ -> throwError $ "non-variable spine element" <+> prettyVTermCtx env t'
  where
    match :: Natural -> VTerm HasMetas ctx -> Maybe (Natural, Lvl ctx)
    match !m (VEmb (VVar l)) = Just (m, l)
    match !m (VQuo s _)      = match' (m + 1) s
    match _  _               = Nothing

    match' :: Natural -> STerm HasMetas ctx -> Maybe (Natural, Lvl ctx)
    match' m (SEmb (SVar l)) = Just (m, l)
    match' m (SQuo s)        = match' (m + 1) s
    match' _ _               = Nothing
...
solve env m sp rhs = do
    ...
    -- rhsT <- either (throwError . fromString . show) return $ quoteTerm UnfoldNone env.size rhs
